...
-- (SYS 계정 접속)
-- VIEW 생성 권한 부여 (SYS -> kh_yhj 계정으로)
GRANT CREATE VIEW TO kh_yhj;

-- VIEW 생성 (kh_yhj 계정으로 접속 후 진행)
CREATE VIEW V_EMP 
...
-- DEPT_COPY2 테이블에서 DEPT_ID, LOZATION_ID 컬럼만 이용해서
-- V_DCOPY2 VIEW 생성
...
VALUES('D0','L2');
--> 가상 테이블 VIEW에 INSERT가 성공한것을 확인함

SELECT * FROM DEPT_COPY2;

-- VIEW에 INSERT 를 수행했지만
...
WHERE TEST_ID = 'TEST500000'; -- 0.015 ~ 0.023
